...
# path_old = r'Z:\Emi\Comparison\2019.xlsx'
# path_new = r'Z:\Emi\Comparison\2020.xlsx'
# path_old = r'C:\Users\EastmanE\Desktop\ProtocolCare\zSiem\Formatted_Protocols_2019.xlsx'
# path_new = r'C:\Users\EastmanE\Desktop\ProtocolCare\zSiem\Formatted_Protocols_20210219135746.xlsx'

def Siemens(oldfilename, newfilename):
            ...
            for row_m in rest_19.index:
                for row_n in rest_20.index:
                    if rest_19.loc[row_m, 'Scan/Recon Type'] == rest_20.loc[row_n, 'Scan/Recon Type']:
                        ...
                        break
                    elif ('Recon' in rest_19.loc[row_m, 'Scan/Recon Type']) & ('Recon' in rest_20.loc[row_n, 'Scan/Recon Type']) & (rest_20.loc[row_n, 'Scan/Recon Type'].split()[0]+rest_20.loc[row_n, 'Scan/Recon Type'].split()[1] == rest_19.loc[row_m, 'Scan/Recon Type'].split()[0]+rest_19.loc[row_m, 'Scan/Recon Type'].split()[1]):
                        for col in rest_19.columns:
                            if rest_19.loc[row_m, col] != rest_20.loc[row_n, col]:
                                #Ignore differences within 5%
                                if (type(rest_19.loc[row_m, col]) == float) & (type(rest_20.loc[row_n, col]) == float):
    ...
    if unchanged_df.empty:
        unchanged_df = pd.DataFrame(data = [['' for i in final_df.columns]], columns = final_df.columns)
    # =============================================================================
    # Format protocols
    # =============================================================================
    ...
    for idx, val in enumerate(collist):
        if val != 1:
            newcollist.append(val)     
            newrowlist.append(rowlist[idx])
